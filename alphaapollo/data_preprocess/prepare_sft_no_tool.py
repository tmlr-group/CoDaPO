...
def main():
    ...
    # Allow "<repo_id>:<config_name>" for HF datasets that require a config
    # (e.g. "openai/gsm8k:main"). HF repo ids cannot contain ':', so splitting is safe.
    if ":" in hf_repo_id and not os.path.exists(hf_repo_id):
        repo_id, config_name = hf_repo_id.split(":", 1)
    else:
        repo_id, config_name = hf_repo_id, None
