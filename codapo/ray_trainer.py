    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for i, batch_dict in enumerate(self.train_dataloader):
                ...
                # CoDaPO: on odd steps, reorganize the (duplicated) batch so that it
                # focuses on the top-difficulty questions selected on the previous even step.
                # Each selected question is repeated `repeat_count` times. This must run
                # before env_kwargs is popped below, since matching is done by question text.
                if (
                    self.config.algorithm.adv_estimator == AdvantageEstimator.CODAPO
                    and i % 2 == 1
                    and self.next_odd_step_selected_questions is not None
                ):
                    repeat_count = self.config.algorithm.get("codapo", {}).get("repeat_count", 4)
                    selected_entity_indices = []
                    if "env_kwargs" in batch.non_tensor_batch:
                        env_kwargs = batch.non_tensor_batch["env_kwargs"]
                        ...
                        if "prompts" in batch.batch:
                            ...
                            original_batch_size = len(env_kwargs) if isinstance(env_kwargs, (list, np.ndarray)) else 0

                        question_idx = 0
                        while len(selected_entity_indices) < original_batch_size:
                            if question_idx < len(self.next_odd_step_selected_questions):
                                question_text = self.next_odd_step_selected_questions[question_idx]
                                matching_indices = []
                                for entity_idx in range(len(env_kwargs)):
                                    env_kwarg = env_kwargs[entity_idx]
                                    if isinstance(env_kwarg, dict) and env_kwarg.get("question") == question_text:
                                        matching_indices.append(entity_idx)
                                for _ in range(repeat_count):
                                    if len(selected_entity_indices) < original_batch_size:
                                        selected_entity_indices.extend(matching_indices)
                                question_idx += 1
                            else:
                                question_idx = 0

                        selected_entity_indices = selected_entity_indices[:original_batch_size]

                    if selected_entity_indices:
                        for key in batch.batch.keys():
                            if isinstance(batch.batch[key], torch.Tensor):
                                batch.batch[key] = batch.batch[key][selected_entity_indices].clone()
                        for key in batch.non_tensor_batch.keys():
                            if isinstance(batch.non_tensor_batch[key], np.ndarray):
                                batch.non_tensor_batch[key] = batch.non_tensor_batch[key][selected_entity_indices]
                            elif isinstance(batch.non_tensor_batch[key], list):
                                batch.non_tensor_batch[key] = [batch.non_tensor_batch[key][idx] for idx in selected_entity_indices]
                        print(f"CoDaPO step {i}: reorganized batch with {len(selected_entity_indices)} entities "
                              f"matching {len(self.next_odd_step_selected_questions)} selected questions")
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        batch = compute_advantage(
                            ...
                            codapo_weight_offset=self.config.algorithm.get("codapo", {}).get("weight_offset", 0.1),
                        )

                        # CoDaPO: on even steps, after advantages/weights are computed,
                        # select the top-25% hardest questions (by codapo weight) and remember
                        # them for the next (odd) step, which sees the same duplicated batch.
                        if (
                            self.config.algorithm.adv_estimator == AdvantageEstimator.CODAPO
                            and i % 2 == 0
                            and "codapo_weights" in batch.batch
                            and "uid" in batch.non_tensor_batch
                        ):
                            percentile = self.config.algorithm.get("codapo", {}).get("percentile", 0.75)  # keep top (1-percentile) by weight
                            weights = batch.batch["codapo_weights"]
                            index = batch.non_tensor_batch["uid"]

                            uid2weights = defaultdict(list)
                            for entity_idx in range(len(weights)):
                                uid2weights[index[entity_idx]].append(weights[entity_idx].item())

                            uid2avg_weight = {}
                            for uid, weights_list in uid2weights.items():
                                weights_tensor = torch.tensor(weights_list)
                                assert torch.allclose(weights_tensor, weights_tensor[0]), \
                                    f"Weights for uid {uid} are not the same: {weights_list}"
                                uid2avg_weight[uid] = weights_list[0]

                            question_weights = torch.tensor(list(uid2avg_weight.values()))
                            weight_threshold = torch.quantile(question_weights, percentile).item()
                            top_uids = [uid for uid, w in uid2avg_weight.items() if w >= weight_threshold]

                            selected_questions = []
                            if "anchor_obs" in batch.non_tensor_batch:
                                anchor_obs = batch.non_tensor_batch["anchor_obs"]
                                uid_array = batch.non_tensor_batch["uid"]
                                selected_uids_set = set(top_uids)
                                uid_to_question = {}
                                for entity_idx in range(len(uid_array)):
                                    uid = uid_array[entity_idx]
                                    if uid in selected_uids_set and uid not in uid_to_question:
                                        if entity_idx < len(anchor_obs):
                                            uid_to_question[uid] = str(anchor_obs[entity_idx])
                                seen_questions = set()
                                for uid in top_uids:
                                    if uid in uid_to_question:
                                        q = uid_to_question[uid]
                                        if q not in seen_questions:
                                            selected_questions.append(q)
                                            seen_questions.add(q)

                            self.next_odd_step_selected_questions = selected_questions
                            print(f"CoDaPO step {i}: prepared {len(selected_questions)} selected questions "
                                  f"for next odd step (top {percentile * 100:.0f}%)")
